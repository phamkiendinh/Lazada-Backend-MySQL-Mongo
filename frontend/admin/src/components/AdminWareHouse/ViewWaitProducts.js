...
;
    ...
    async function deleteProduct(id) {
        const data = await fetch(`http://localhost:3001/admin/warehouse/products/waiting/${id}`, {
            method: "DELETE"
        })
        .then(res => res.json())
        .then(data => {
        ...
        return (
            <div className="row">
                ...
                <div className="col-8 mt-2 d-flex justify-content-center">
                    <h1 className="fw-bold">No Products Waiting</h1>
                </div>
            </div>
        );
    }
                    ...
                    return (
                        <div className="border border-2 border-primary mb-2" key={item.id}>
                            ...
                            <div className="mb-2 d-flex justify-content-center" key={item.id}> 
                                <button className="btn btn-primary mx-2" onClick={() => moveProduct(item.id, item.wid)}>
                                    Move Product
                                </button>
...
export async function loadWaitProducts({request, params}) {
    const data = await
    fetch(`http://localhost:3001/admin/warehouse/products/waiting`)
    .then(res => res.json())
    .then(data => {
        // console.log(data);
        return data;
...
export default ViewWaitProducts;